...
router.post("/get-request", function(req, res, next) {
  pool.getConnection((err, connection) => {
    if(err) throw err;
      ...
      connection.query(`SELECT gr.group_no as no, gr.color as color FROM invite inv, scheduler.group gr where gr.group_no = inv.group_no AND user_no = ${req.session.user.no}`, (err, result) => {
        if(err) throw err;
